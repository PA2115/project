...
//Everything Below has been implemented into postman and working
...
app.post('/calcCPA',  async (req, res) => {
    ...
    try{
        const retrieved = await pool.query('SELECT 1000 / COUNT(transaction.transaction_id)  AS example_count, offer.offer_name, industries.industries_name FROM transaction INNER JOIN offer ON offer.offer_id = transaction.offer_id INNER JOIN industries ON industries.industries_id = offer.industries_id GROUP BY offer.offer_name, industries.industries_name')
        console.log(res.json(retrieved.rows))
